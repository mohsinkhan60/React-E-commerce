    ...
    <Section crosses>
      <div className="container lg:flex">
        ...
        <div className="lg:ml-auto xl:w-[38rem] ">
          <p className="body-2 mb-4 mt-8 text-n-4 md:mb-16 lg:mb-32 lg:w-[22rem] lg:mx-auto">
            {collabText}
          </p>
            ...
            <div className="flex w-60 aspect-square m-auto border border-n-6 rounded-full">
              <div className="w-[6rem] aspect-square m-auto p-[0.2rem] bg-conic-gradient rounded-full">
                <div className="flex items-center justify-center rounded-full w-full h-full bg-n-8">
                  <img src={brainwaveSymbol} width={48} height={48} alt="img" />
                </div>
              </div>
            </div>
            <ul>
              {collabApps.map((item, index) => (
                <li key={item.id} className={`absolute top-0 left-1/2 h-1/2 -ml-[1.6rem] origin-bottom rotate-${index * 45}`}>
                  <div className={`relative -top-[1.6rem] flex w-[3.2rem] h-[3.2rem] bg-n-7 border border-n-1/15 rounded-xl -rotate-[index * 45]`}>
                    <img src={item.icon} alt="img" width={item.width} height={item.height} className="m-auto" />
                  </div>
                </li>
              ))}
            </ul>
